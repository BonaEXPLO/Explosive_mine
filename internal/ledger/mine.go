...
// Exported for trusted internal packages (halving, scan, p2p, transaction, etc.).
...
func Mine(db *Ledger, miner *Miner, state *MinerState, donationPercent float64) error {
        now := time.Now().Unix()
// Critical V3 validation: prove possession of consciousness fingerprint
        if err := EnsureMinerSignature(miner); err != nil {
                return fmt.Errorf("miner identity not verified — invalid or missing sacred words: %w", err)
        }
