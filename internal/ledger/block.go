...
// ---------------- Validation ----------------
func (b *Block) Validate(expectedPrev string) error {
    if b.BlockHash != b.ComputeFinalHash() {
        return fmt.Errorf("invalid block hash")
    }
    if b.Header.Height == 0 && expectedPrev != "" {
        return fmt.Errorf("genesis block should not have previous hash")
    }
    if b.Header.Height > 0 && b.Header.PrevHash != expectedPrev {
...
func ComputeMerkleRoot(txs []Transaction) string {
    ...
    for _, tx := range txs {
        h := sha3.Sum256([]byte(fmt.Sprintf("%v", tx)))
        hashes = append(hashes, h[:]...)
    }
    final := sha3.Sum256(hashes)
    return hex.EncodeToString(final[:])
}

// ApplyBlock validates then queues the block for storage.
func (l *Ledger) ApplyBlock(blk *Block) error {
    ...
    if err := l.IterateBlocks(func(b *Block) error {
        ...
        return fmt.Errorf("failed to retrieve last block: %v", err)
    }

    if err := blk.Validate(lastHash); err != nil {
...
// DumpLedger prints all blocks in the database and detects corrupted blocks.
// Useful for debugging and verifying blockchain integrity.
func (l *Ledger) DumpLedger() error {
    ...
    err := l.IterateBlocks(func(b *Block) error {
        // Recompute the block hash to check integrity
        computedHash := b.ComputeFinalHash()
        isCorrupt := false

        if b.BlockHash != computedHash {
        ...
        // Check previous hash consistency (except for genesis block)
        ...
        // Update prevHash for next block check
