...
func SyncWalletBalances() error {
	...
	if err := walletDB.SetBalance(currentWallet.Address, wallet.WalletBalance{
		EXPLO: float64(bal.EXPLO) / float64(ledger.PastaboPerEXPLO),
		IMANI: float64(bal.IMANI) / float64(ledger.PastaboPerEXPLO),
	}); err != nil {
...
func flowSendEXPLO() error {
	...
	if fromBal.EXPLO < uint64(amt*float64(ledger.PastaboPerEXPLO)+0.5) {
		return fmt.Errorf("insufficient EXPLO balance")
	}

	// 3️⃣ Create transaction
	feeFloat := 0.001 * float64(ledger.PastaboPerEXPLO)
	feePastabo := uint64(math.Round(feeFloat))

	amountFloat := amt * float64(ledger.PastaboPerEXPLO)
	amountPastabo := uint64(math.Round(amountFloat))

	if amountFloat > float64(math.MaxUint64) {
		return fmt.Errorf("amount too large – would overflow Pastabo")
	}

	if amountPastabo == 0 && amt > 0 {
		return fmt.Errorf("amount too small – below minimum precision")
	}
